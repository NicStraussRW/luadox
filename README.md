...
dropped.  Each undocumented member is reported under its own `undocumented-enum-fields`
diagnostic category, so a project whose enums are generated can accept them with
`allow_incomplete = undocumented-enum-fields` without relaxing any other check.  An `@enum`
...
* `undocumented-enum-fields`: an `@enum` member has no doc comment.  Reported per member,
