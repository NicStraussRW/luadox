...
class Parser:
    ...
    def validate_enums(self) -> None:
        ...
        with a native enum representation can treat membership as closed.  We report an
        @enum with no members at all (the tag landed on something that isn't a table of
        integer constants) and any member not assigned an integer literal.  A member
        assigned twice surfaces as a `conflicts` diagnostic when it is added, and a nested
        @table/@enum is rejected during parsing.
        """
