...
class Parser:
        ...
        while True:
            ...
            if comment and ref:
                ...
                if parse_next_code_line:
                    ...
                    if ref is None:
                        # A name assigned inside an @enum or an explicit @section is a member of
                        # it even without a doc comment.  Both are declared memberships: the enum
                        # mirrors a C++ enumeration, and a section names the group it contains.
                        # Their sources are usually autogenerated from C++, whose Doxygen comments
                        # are often absent, so membership follows the assignment rather than the
                        # documentation.  Emit it as a value-only field, and report it, so an
                        # undocumented member is visible in the output instead of disappearing
                        # from it -- a plain @table still skips an undocumented field.
                        member_scope = scopes[-1]
                        in_enum = (isinstance(member_scope, TableRef)
                                   and member_scope.flags.get('enum'))
                        in_section = isinstance(collection, SectionRef)
                        if in_enum or in_section:
                            ...
                            # (`__index`), so it is not swept in undocumented; documenting one
                            # explicitly still emits it, as it always did.
                            internal = bool(name) and name.rsplit('.', 1)[-1].startswith('__')
                            if name and not internal and (in_enum or qualified):
                                field = FieldRef(
                                    self.refs, file=path, line=n, scopes=scopes[:],
                                ...
                                if in_section:
                                    self.diagnostics.add(
                                        'undocumented-section-members',
                                        '{} has no doc comment'.format(name),
                                        path, n)
                        continue
    ...
    def validate_enums(self) -> None:
        ...
        Whether a member is documented is the source's to decide: an undocumented member
        is not malformed, just undocumented, so each one is reported at its own location
        under the granular `undocumented-enum-members` category rather than under a generic
        `undocumented` category or promoted to a `structure` error.  A project whose enums
        are autogenerated -- the C++ enumerators often carrying no comment -- can then accept
        `allow_incomplete = undocumented-enum-members` without loosening any other check, and
        a partly documented enum no longer forces an all-or-nothing choice.

        We still report as `structure` an @enum with no members at all (the tag landed on
        something that isn't a table of integer constants) and any member not assigned an
        integer literal, because those are malformed rather than merely undocumented.
        ...
        for colref in self.parsed[TableRef]:
            ...
            for member in members:
                # A member is documented iff it carried a doc comment (raw_content); a
                # value-only member synthesized from a bare assignment has none.
                if not member.raw_content:
                    self.diagnostics.add(
                        'undocumented-enum-members',
                        '@enum member {} has no doc comment'.format(member.name),
